...
namespace Eraser.Util
{
	public sealed class CryptAPI : IDisposable
	{
		...
		private CryptAPI()
		{
			...
			if (NativeMethods.CryptAcquireContext(out handle, string.Empty, IntelDefaultProvider,
				NativeMethods.PROV_INTEL_SEC, 0))
			{
				return;
			}
			else if (NativeMethods.CryptAcquireContext(out handle, string.Empty, string.Empty,
				NativeMethods.PROV_RSA_FULL, 0))
			{
				return;
			}
		...
		{
			return NativeMethods.CryptGenRandomInternal(instance.handle,
	...
	internal class SafeCryptHandle : SafeHandle
	{
		...
		{
			CryptAPI.NativeMethods.CryptReleaseContext(handle, 0u);
			handle = IntPtr.Zero;
			return true;
		}
	}
